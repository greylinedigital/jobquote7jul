...
import { ChevronLeft, ChevronDown, Plus, Zap, User, FileText, Pencil, DollarSign } from 'lucide-react-native';
...
export default function FastQuote() {
  ...
  const [quoteLimitInfo, setQuoteLimitInfo] = useState({ canCreate: true, remaining: 0 });
  ...
  const generateQuote = async () => {
    ...
    // Check quote limits for non-premium users
    if (!isPremium) {
      const limitInfo = await checkQuoteLimit();
    ...
    console.log('User Hourly Rate:', currentHourlyRate);
    ...
    try {
      ...
      if (quoteItems.length > 0) {
        ...
        console.log('Quote items inserted successfully:', quoteItems.length);
      }

      // Refresh quote limits
      await checkQuoteLimits();

      console.log('Navigating to quote preview:', quote.id);
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        {!isPremium && (
          <View style={styles.usageIndicator}>
            <Text style={styles.usageText}>
              {quoteLimitInfo.remaining > 0 
                ? `${quoteLimitInfo.remaining} quote${quoteLimitInfo.remaining === 1 ? '' : 's'} remaining this week`
                : 'Weekly quote limit reached'
              }
            </Text>
            {quoteLimitInfo.remaining <= 1 && (
              <TouchableOpacity
                style={styles.upgradeHint}
                onPress={() => setSubscriptionModalVisible(true)}
              >
                <Text style={styles.upgradeHintText}>Upgrade for unlimited quotes</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
      ...
      <View style={styles.fixedButtonContainer}>
        <TouchableOpacity
          style={[
            ...
            (loading || (!isPremium && !quoteLimitInfo.canCreate)) && styles.generateButtonDisabled
          ]}
          onPress={generateQuote}
          disabled={loading || (!isPremium && !quoteLimitInfo.canCreate)}
        >
          {loading ? (
          ...
          <Text style={styles.generateButtonText}>
            {loading ? 'GENERATING...' : 
             (!isPremium && !quoteLimitInfo.canCreate) ? 'LIMIT REACHED' : 
             'GENERATE QUOTE'}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <LimitReachedModal
        ...
        type="quotes"
        remaining={quoteLimitInfo.remaining}
      />
